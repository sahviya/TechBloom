...
export default function PDFViewer({ book, onClose }: PDFViewerProps) {
  ...
  useEffect(() => {
    saveData();
  }, [currentPage, readingTheme, fontSize, bookmarks, highlights, zoom]);
      ...
      <DialogContent className="max-w-7xl w-full h-[90vh] p-0">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            >
              <iframe
                src={`${book.pdfUrl}#page=${currentPage}&toolbar=0&navpanes=0&scrollbar=1&zoom=${zoom}&view=FitH`}
                className="w-full h-full border-0"
                title={book.title}
                onLoad={() => {
                  setLoading(false);
                  // Try to get total pages from the PDF
